...
export default function App() {
  ...
  const [isPartnerMode, setIsPartnerMode] = useState(() => {
    ...
    const savedAdminMode = localStorage.getItem('gula_admin_logged_in') === 'true';
  ...
  const [isAdminUnlocked, setIsAdminUnlocked] = useState(() => {
    return localStorage.getItem('gula_admin_logged_in') === 'true';
  });
  const isAdmin = isAdminUnlocked;

  const handleAdminLogout = () => {
    localStorage.removeItem('gula_admin_logged_in');
    ...
    return (
      <div className="bg-slate-950 min-h-screen text-slate-100 antialiased selection:bg-amber-500 selection:text-slate-950">
        <PartnerDashboardView 
          ...
          onUnlockAdmin={() => {
            localStorage.setItem('gula_admin_logged_in', 'true');
