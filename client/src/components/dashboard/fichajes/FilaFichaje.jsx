...
export default function FilaFichaje({ entrada, avatar, turno = null, enCurso = false, admin = false, onEditar, onEliminar, retraso = 0 }) {
  ...
  const hora = entrada.timestamp ? formatTime(entrada.timestamp) : entrada.timeFormatted;
  ...
  const diaCorto = empezoOtroDia ? inicioTurno.toLocaleDateString('es-ES', { weekday: 'short', day: 'numeric' }).replace('.', '') : '';
